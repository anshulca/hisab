...
interface I1FinalHisabCheckProps {
  onNavigate: (section: string) => void;
  onFinalize: () => void;
}

export function I1FinalHisabCheck({ onNavigate, onFinalize }: I1FinalHisabCheckProps) {
  ...
  const handleDownload = useGuardedDownload(async () => {
    if (normalizedData) await generateItr1Pdf(normalizedData);
  });
    ...
    <div className="section">
      <div className="container" style={{ maxWidth: 900 }}>
        <div className="section-head">
          <h2 className="section-title">Final Hisab Check — ITR-1 (SAHAJ)</h2>
        ...
        <div className="card" style={{ marginTop: 24 }}>
          ...
          <div className="table-wrap" style={{ marginTop: 14 }}>
            <table className="mini-table">
              <tbody>
                <tr><td>Gross Total Income (sum of heads)</td><td style={{ textAlign: 'right' }}>{formatINR(report.income.grossTotalCalculated)}</td></tr>
                <tr><td>Total Income (as per ITR)</td><td style={{ textAlign: 'right' }}>{report.income.totalIncomeReported !== 0 ? formatINR(report.income.totalIncomeReported) : '—'}</td></tr>
                <tr><td>Taxable Income (HISAB)</td><td style={{ textAlign: 'right' }}>{formatINR(report.income.taxableCalculated)}</td></tr>
                <tr><td>Total Tax (HISAB)</td><td style={{ textAlign: 'right' }}>{formatINR(report.taxComputed.grossLiability)}</td></tr>
                <tr><td>Total Taxes Paid</td><td style={{ textAlign: 'right' }}>{formatINR(report.taxesPaid.total)}</td></tr>
                <tr><td><strong>Net Tax Payable</strong></td><td className="highlight" style={{ textAlign: 'right' }}><strong>{formatINR(report.taxComputed.netPayable)}</strong></td></tr>
              </tbody>
            </table>
          </div>
        </div>
